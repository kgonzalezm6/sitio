...
<template>
    <div v-if="open"
        ...
        <div class="flex font-bold text-white justify-between items-center py-2 px-3 bg-clip-padding border-b border-gray-300 rounded-t-lg ">
            ...
            <icon v-else-if="type === 'dark'" icon="fa-solid fa-circle-check" class="text-xl"/>
        ...
        <div class="p-3 rounded-b-lg break-words  text-justify"
            :class="{
                'bg-blue-100 text-blue-500'   : type === 'primary',
                'bg-green-100 text-green-500'  : type === 'success',
                'bg-red-100 text-red-500'    : type === 'danger',
                'bg-yellow-50 text-yellow-600' : type === 'warning',
                'bg-gray-100 text-gray-800' : type === 'dark',
            }">
            {{ props.message }}
        </div>
    </div>
