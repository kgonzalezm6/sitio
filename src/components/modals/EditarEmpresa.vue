    ...
    <div>
        <modal :open="store.opcion_editar" icon="edit" class="w-[70%]">
                <div class="my-5" v-if="store.loading_opciones">
                    ...
                    <text-field type="url" option="label" title="sitio" v-model="store.registro.sitio"/>
                    <text-field acceptedTypes="image/*" option="file" title="imagen" 
                    class="w-full"  @change="handleFileChange"/>
                    <div class="flex justify-center">
                        <img  :src="imageUrl(store.registro.imagen)" :alt="store.registro.nombre" class="h-[5rem]">
                    </div>
                </div>
